...
def read():
    ...
    print(len(data["icsf"]["embed"]), "|", len(data["ccsf"]["embed"]))
    exit(0)
...
def plot_embeds(ax, data, mode, legend=None):
    ...
    ax.scatter(data["embed"][:data["split"], 0], data["embed"][:data["split"], 1], s=1, c="blue", label="train", alpha=0.5)
    ax.scatter(data["embed"][data["split"]:, 0], data["embed"][data["split"]:, 1], s=1, c="orange", label="test", alpha=0.5)
    ...
    ax.set_title(f"{'ECFP4' if mode[0] == 'e' else 'ESM'} embeddings of\n{'identity' if mode[1] =='i' else 'cluster'}-based split")
...
def plot_full(data):
    ...
    axis = 0
    rand, icse, icsf, icd, ccse, ccsf, ccd = data["rand"], data["icse"], data["icsf"], data["icd"], data["ccse"], data["ccsf"], data["ccd"]
    ...
    for tech, name in [(rand, "Random"), (icse, "Drug ID-based"), (icsf, "Prot ID-based"), (icd, "ID-based 2D"), (ccse, "Drug cluster-based"), (ccsf, "Prot cluster-based"), (ccd, "cluster-based 2D")]:
