...
def read_single_data(name, path, encodings):
    ...
    for r in range(RUNS):
        ...
        results_path = path / f"split_{r}" / "results" / "valid_markdowntable.txt"
        if os.path.exists(results_path):
            metric.append(read_markdowntable(results_path))
...
def read_data():
    ...
    data = {n: read_single_data(n, Path("data_scip_improved") / n, encodings) for n in ["R", "ICSe", "ICSf", "ICD", "CCSe", "CCSf", "CCD"]}
...
def plot_full(data):
    ...
    axis = 0
    rand, icse, icsf, icd, ccse, ccsf, ccd = data["R"], data["ICSe"], data["ICSf"], data["ICD"], data["CCSe"], data["CCSf"], data["CCD"]
...
def analyze():
    pkl_name = f"read_data_{'umap' if USE_UMAP else 'tsne'}.pkl"
