...
def save_datasail_splits(base: Path, df: pd.DataFrame, key: str, techniques: List[str], e_splits: Optional[Dict] = None, inter_splits: Optional[Dict] = None) -> None:
    """
    ...
    for tech in techniques:
        for run in range(RUNS):
            ...
            for label in ["train", "test"]:
                ...
                df[sub].to_csv(path / f"{label}.csv")
...
def embed_smiles(
    ...
    try:
        if smile not in drug_embeds:
            ...
            drug_embeds[smile] = np.ndarray(list(AllChem.GetMorganFingerprintAsBitVect(mol, 2, nBits=n_bits)))
    except:
        drug_embeds[smile] = None
...
def telegram(message: str = "Hello World") -> None:
    """
