...
def old():
    for tool in ["datasail", "deepchem"]:
        for ds_name in sorted(os.listdir(Path("../MPP") / "deepchem" / "sdata"), key=lambda x: DATASETS[x][3]):
    ...
    ds2idx = {n: i for i, n in
              enumerate(sorted(os.listdir(Path("../MPP") / "deepchem" / "sdata"), key=lambda x: DATASETS[x][3]))}
